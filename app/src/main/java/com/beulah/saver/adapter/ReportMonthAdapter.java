...
public class ReportMonthAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        name.setText(RupayeUtil.GetMonthShort((int) highestMonthExpense.get(position + 1)[1] + 1) + " " + year + RupayeUtil.getInstance().GetPurePercentString(highestMonthExpense.get(position + 1)[4] * 100));
        expense.setText(RupayeUtil.getInstance().GetInMoney((int) highestMonthExpense.get(position + 1)[3]));
        records.setText(RupayeUtil.getInstance().GetInRecords((int) highestMonthExpense.get(position + 1)[5]));
