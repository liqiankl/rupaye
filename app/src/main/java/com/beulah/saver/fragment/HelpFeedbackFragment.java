...
import com.beulah.saver.model.Feedback;
...
public class HelpFeedbackFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        input.setTypeface(RupayeUtil.getInstance().typefaceLatoLight);
        help = (TextView)view.findViewById(R.id.helper);
        help.setTypeface(RupayeUtil.getInstance().typefaceLatoLight);
        number = (TextView)view.findViewById(R.id.number);
        number.setTypeface(RupayeUtil.getInstance().typefaceLatoLight);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (exceed) {
                    ...
                    RupayeUtil.getInstance().showToast(RupayeApplication.getAppContext(), RupayeApplication.getAppContext().getResources().getString(R.string.help_feedback_sent));
                    Feedback feedback = new Feedback();
                    feedback.setContent(input.getText().toString());
                    feedback.save(RupayeApplication.getAppContext(), new SaveListener() {
                        @Override
                        public void onSuccess() {
                            RupayeUtil.getInstance().showToast(RupayeApplication.getAppContext(), RupayeApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_successfully));
                        }

                        @Override
                        public void onFailure(int code, String arg0) {
                            RupayeUtil.getInstance().showToast(RupayeApplication.getAppContext(), RupayeApplication.getAppContext().getResources().getString(R.string.help_feedback_sent_fail));
                        }
                    });
                }
            }
    ...
    private void setNumberText() {
        ...
        if (chineseIsDoubleCount) {
            count = RupayeUtil.getInstance().textCounter(input.getText().toString());
        } else {
