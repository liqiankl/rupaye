...
public class TodayViewRecyclerViewAdapter
    ...
    private ArrayList<RupayeRecord> allData;
    ...
    public TodayViewRecyclerViewAdapter(int start, int end, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            for (int i = 0; i < size; i++) {
                RupayeRecord RupayeRecord = allData.get(i);
                ...
                if (axis_date == Calendar.DAY_OF_WEEK) {
                    if (RupayeUtil.WEEK_START_WITH_SUNDAY)
                        originalTargets[RupayeRecord.getCalendar().get(axis_date) - 1]
                                += RupayeRecord.getMoney();
                    else originalTargets[(RupayeRecord.getCalendar().get(axis_date) + 5) % 7]
                                += RupayeRecord.getMoney();
                } else if (axis_date == Calendar.DAY_OF_MONTH) {
                    originalTargets[RupayeRecord.getCalendar().get(axis_date) - 1]
                            += RupayeRecord.getMoney();
                } else {
                    originalTargets[RupayeRecord.getCalendar().get(axis_date)]
                            += RupayeRecord.getMoney();
                }
            }
    ...
    public void onBindViewHolder(final viewHolder holder, final int position) {
        ...
        switch (getItemViewType(position)) {
            case TYPE_HEADER:
                ...
                holder.expanseSum.setText(RupayeUtil.GetInMoney((int) Sum));

                holder.date.setTypeface(RupayeUtil.GetTypeface());
                holder.dateBottom.setTypeface(RupayeUtil.GetTypeface());
                holder.expanseSum.setTypeface(RupayeUtil.typefaceLatoLight);

                if (IS_EMPTY) {
                    holder.emptyTip.setVisibility(View.VISIBLE);
                    holder.emptyTip.setText(RupayeUtil.GetTodayViewEmptyTip(fragmentPosition));
                    ...
                    if (!(fragmentPosition == TODAY || fragmentPosition == YESTERDAY)) {
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            ...
                            SubcolumnValue value = new SubcolumnValue(
                                    originalTargets[i], RupayeUtil.GetRandomColor());
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            axisValueList.add(
                                    new AxisValue(i).setLabel(RupayeUtil.GetAxisDateName(axis_date, i)));
                        }
                        ...
                        public void onValueSelected(int p, SliceValue sliceValue) {
                            ...
                            Snackbar snackbar =
                                    Snackbar
                                            ...
                                            .margin(15, 15)
                                            .backgroundDrawable(RupayeUtil.GetSnackBarBackground(
                                                    fragmentPosition - 2))
                                            .text(text)
...
// histogram data///////////////////////////////////////////////////////////////////////////////////
                                ...
                                for (int i = Expanse.get(tagId).size() - 1; i >= 0; i--) {
                                    RupayeRecord RupayeRecord = Expanse.get(tagId).get(i);
                                    if (axis_date == Calendar.DAY_OF_WEEK) {
                                        if (RupayeUtil.WEEK_START_WITH_SUNDAY) {
                                            targets[RupayeRecord.getCalendar().get(axis_date) - 1]
                                                    += RupayeRecord.getMoney();
                                        } else {
                                            targets[(RupayeRecord.getCalendar().get(axis_date) + 5) % 7]
                                                    += RupayeRecord.getMoney();
                                        }
                                    } else if (axis_date == Calendar.DAY_OF_MONTH) {
                                        targets[RupayeRecord.getCalendar().get(axis_date) - 1]
                                        ...
                                        targets[RupayeRecord.getCalendar().get(axis_date)]
                                                += RupayeRecord.getMoney();
                                    }
                                }
...
// set value touch listener of histogram////////////////////////////////////////////////////////////
                        holder.histogram.setOnValueTouchListener(
                            ...
                            public void onValueSelected(int columnIndex,
                                ...
                                String text = RupayeUtil.GetSpendString((int) value.getValue());
                                if (tagId != -1)
                                    // belongs a tag
                                    if ("zh".equals(RupayeUtil.GetLanguage()))
...
// set the listener of the show all button//////////////////////////////////////////////////////////
            ...
            case TYPE_BODY:
                ...
                holder.tagImage.setImageResource(
                        RupayeUtil.GetTagIcon(allData.get(position - 1).getTag()));
                ...
                holder.cell_date.setTypeface(RupayeUtil.typefaceLatoLight);
                ...
                holder.layout.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        dialog = new MaterialDialog.Builder(mContext)
                                .icon(RupayeUtil.GetTagIconDrawable(allData.get(position - 1).getTag()))
...
// set the listener of the check button on the snack bar of histogram///////////////////////////////
    private class mActionClickListenerForHistogram implements ActionClickListener {
        @Override
        public void onActionClicked(Snackbar snackbar) {
            ArrayList<RupayeRecord> shownRupayeRecords = new ArrayList<>();
            int index = timeIndex;
            if (axis_date == Calendar.DAY_OF_WEEK) {
                if (RupayeUtil.WEEK_START_WITH_SUNDAY) index++;
                else
                    if (index == 6) index = 1;
                    else index += 2;
            }
            ...
            if (tagId != -1) {
                for (int i = 0; i < Expanse.get(tagId).size(); i++)
                    if (Expanse.get(tagId).get(i).getCalendar().get(axis_date) == index)
                        shownRupayeRecords.add(Expanse.get(tagId).get(i));
            } else {
                for (int i = 0; i < allData.size(); i++)
                    if (allData.get(i).getCalendar().get(axis_date) == index)
                        shownRupayeRecords.add(allData.get(i));
            }
            ((FragmentActivity)mContext).getSupportFragmentManager()
                    .beginTransaction()
                    .add(new RecordCheckDialogFragment(
        ...
        switch (fragmentPosition) {
            ...
            case THIS_WEEK:
                ...
                return mContext.getResources().getString(R.string.on)
                        + RupayeUtil.GetWeekDay(timeIndex);
            case LAST_WEEK:
                // 在上周一
                // on last Monday
                return mContext.getResources().getString(R.string.on)
            ...
            case THIS_MONTH:
                ...
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
                        RupayeUtil.GetMonthShort(month) + RupayeUtil.GetWhetherBlank() +
                        (timeIndex + 1) + RupayeUtil.GetWhetherFuck();
            case LAST_MONTH:
                // 在1月1日
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
            ...
            case THIS_YEAR:
                if ("zh".equals(RupayeUtil.GetLanguage()))
                    ...
                    return mContext.getResources().getString(R.string.in) +
                            ...
                            RupayeUtil.GetMonthShort(timeIndex + 1);
                else
                    // in Jan. 1
                    return mContext.getResources().getString(R.string.in) +
                            RupayeUtil.GetMonthShort(timeIndex + 1) + " " +
...
// set the dateString of this fragment//////////////////////////////////////////////////////////////
    private void setDateString() {
        ...
        Calendar yesterday = RupayeUtil.GetYesterdayLeftRange(today);
        basicTodayDateString = "--:-- ";
        basicTodayDateString += RupayeUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                + " " + today.get(Calendar.DAY_OF_MONTH) + " " +
                today.get(Calendar.YEAR);
        basicYesterdayDateString = "--:-- ";
        basicYesterdayDateString += RupayeUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
        ...
        switch (fragmentPosition) {
            ...
            case YESTERDAY:
                ...
                break;
            case THIS_WEEK:
                Calendar leftWeekRange = RupayeUtil.GetThisWeekLeftRange(today);
                Calendar rightWeekRange = RupayeUtil.GetThisWeekRightShownRange(today);
                dateString = RupayeUtil.GetMonthShort(leftWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftWeekRange.get(Calendar.YEAR) + " - " +
                        RupayeUtil.GetMonthShort(rightWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case LAST_WEEK:
                Calendar leftLastWeekRange = RupayeUtil.GetLastWeekLeftRange(today);
                Calendar rightLastWeekRange = RupayeUtil.GetLastWeekRightShownRange(today);
                dateString
                        = RupayeUtil.GetMonthShort(leftLastWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftLastWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftLastWeekRange.get(Calendar.YEAR) + " - " +
                        RupayeUtil.GetMonthShort(rightLastWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case THIS_MONTH:
                dateString = RupayeUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                        + " " + today.get(Calendar.YEAR);
                dateShownString
                ...
                break;
            case LAST_MONTH:
                Calendar lastMonthCalendar = RupayeUtil.GetLastMonthLeftRange(today);
                dateString
                        = RupayeUtil.GetMonthShort(lastMonthCalendar.get(Calendar.MONTH) + 1)
            ...
            case THIS_YEAR:
                ...
                break;
            case LAST_YEAR:
                Calendar lastYearCalendar = RupayeUtil.GetLastYearLeftRange(today);
    ...
    private String getAllDataDialogTitle() {
        ...
        if ("zh".equals(RupayeUtil.GetLanguage())) {
            ...
            postfix = RupayeUtil.GetSpendString((int)Sum);
        } else {
            prefix = RupayeUtil.GetSpendString((int)Sum);
            postfix = "";
        }
        switch (fragmentPosition) {
            case TODAY:
