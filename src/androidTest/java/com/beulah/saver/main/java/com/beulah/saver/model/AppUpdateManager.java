...
public class AppUpdateManager {
    ...
    private static final String FILE_NAME = FILE_PATH + "Rupaye.apk";
    ...
    public void checkUpdateInfo(final Boolean showInfo) {
        ...
        query.addWhereGreaterThan("version", RupayeApplication.VERSION);
        query.setLimit(Integer.MAX_VALUE);
        query.findObjects(RupayeApplication.getAppContext(), new FindListener<APK>() {
            @Override
            public void onSuccess(final List<APK> object) {
                if (object.size() == 0 && showInfo) {
                    RupayeUtil.showToast(context, context.getResources().getString(R.string.is_newest_version), SuperToast.Background.BLUE);
                }
                BmobQuery<APK> tooOldQuery = new BmobQuery<>();
                tooOldQuery.addWhereEqualTo("version", RupayeApplication.VERSION);
                tooOldQuery.setLimit(1);
                tooOldQuery.findObjects(RupayeApplication.getAppContext(), new FindListener<APK>() {
                    @Override
